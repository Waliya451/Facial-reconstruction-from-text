...
class GeneratorBlock(nn.Module):
    def __init__(self, latent_dim, input_channels, filters, upsample = True, upsample_rgb = True, rgba = False):
        ...
        self.to_noise1 = nn.Linear(16, 512)
        self.conv1 = Conv2DMod(input_channels, filters, 3)
        
        self.to_style2 = nn.Linear(latent_dim, filters)
        # self.to_noise2 = nn.Linear(1, filters)
        self.to_noise2 = nn.Linear(16, 512)
    ...
    def forward(self, x, prev_rgb, istyle, inoise):
        device = x.device  # Ensure all tensors move to the same device

        if exists(self.upsample):
            x = self.upsample(x)

        # Fetch the correct noise tensor for the current resolution
        current_resolution_index = int(np.log2(x.shape[2])) - 2  # Adjust index based on resolution
        inoise = inoise[current_resolution_index].to(device)  # Move inoise to same device as x

        # Ensure inoise has the correct shape before passing to Linear layer
        inoise = inoise.view(inoise.shape[0], -1).to(device)  # Flatten and move to correct device
        expected_noise_features = self.to_noise1.in_features  # Should be 16

        if inoise.shape[1] != expected_noise_features:
            inoise = F.adaptive_avg_pool1d(inoise.unsqueeze(1), expected_noise_features).squeeze(1).to(device)

        print(f"inoise shape before Linear layer: {inoise.shape}")
        print(f"Expected input size for Linear layer: {expected_noise_features}")

        # Pass through noise Linear layers
        noise1 = self.to_noise1(inoise.to(device))
        noise2 = self.to_noise2(inoise.to(device))

        # Ensure noise1 and noise2 are 4D before slicing
        if noise1.dim() == 2:  # If noise1 is (batch, channels)
            print('📌 Reshaping noise1 from 2D to 4D')
            noise1 = noise1.view(noise1.shape[0], noise1.shape[1], 1, 1).expand(-1, -1, x.shape[2], x.shape[3])

        if noise2.dim() == 2:  # If noise2 is (batch, channels)
            print('📌 Reshaping noise2 from 2D to 4D')
            noise2 = noise2.view(noise2.shape[0], noise2.shape[1], 1, 1).expand(-1, -1, x.shape[2], x.shape[3])

        # 🔹 **Fix: Ensure noise channels match x’s channels**
        if noise1.shape[1] > x.shape[1]:  
            print(f"⚠️ Adjusting noise from {noise1.shape[1]} to {x.shape[1]} at resolution {x.shape[2]}x{x.shape[3]}")
            noise1 = noise1[:, :x.shape[1], :, :]
            noise2 = noise2[:, :x.shape[1], :, :]

        print("x shape:", x.shape)
        print("noise1 shape after adjustment:", noise1.shape)
        print("noise2 shape after adjustment:", noise2.shape)

        # Ensure istyle is reshaped to [batch_size, 512] before passing through Linear layers
        print("istyle shape before Linear layer:", istyle.shape)
        istyle = istyle.view(istyle.shape[0], -1).to(device)

        # If `istyle` does not have 512 features, expand or project it to match
        if istyle.shape[1] != 512:
            istyle = F.interpolate(istyle.unsqueeze(1), size=(512,), mode='linear', align_corners=False).squeeze(1).to(device)

        print("istyle shape after reshaping:", istyle.shape)

        # Ensure style layers match x's channels
        style1 = self.to_style1(istyle.to(device))
        style2 = self.to_style2(istyle.to(device))    

        # Pass through first convolution
        x = self.conv1(x, style1)

        # Handle specific resolutions where channel adjustments are required
        if x.shape[2] == 128 and x.shape[1] == 256:
            print("⚠️ Adjusting noise for 128x128 resolution after conv1")
            noise1 = noise1[:, :256, :, :]

        if x.shape[2] == 256 and x.shape[1] == 128:
            print("⚠️ Adjusting noise for 256x256 resolution after conv1")
            noise1 = noise1[:, :128, :, :]

        # Handle 512x512 resolution
        if x.shape[2] == 512 and x.shape[1] == 64:  
            print("⚠️ Adjusting noise for 512x512 resolution after conv1")
            noise1 = noise1[:, :64, :, :]  

        if x.shape[2] == 1024 and x.shape[1] == 32:  
            print("⚠️ Adjusting noise for 1024x1024 resolution after conv1")
            noise1 = noise1[:, :32, :, :]  

        # Apply activation with noise
        ...
        if x.shape[2] == 128 and x.shape[1] == 256:
            print("⚠️ Adjusting noise for 128x128 resolution after conv2")
            noise2 = noise2[:, :256, :, :]

        if x.shape[2] == 256 and x.shape[1] == 128:
            print("⚠️ Adjusting noise for 256x256 resolution after conv2")
            noise2 = noise2[:, :128, :, :]

        # Handle 512x512 resolution after conv2
        if x.shape[2] == 512 and x.shape[1] == 64:
            print("⚠️ Adjusting noise for 512x512 resolution after conv2")
            noise2 = noise2[:, :64, :, :]  

        if x.shape[2] == 1024 and x.shape[1] == 32:
            print("⚠️ Adjusting noise for 1024x1024 resolution after conv2")
            noise2 = noise2[:, :32, :, :] 
        ...
        # Generate RGB output
        rgb = self.to_rgb(x, prev_rgb, istyle.to(device))
    ...
    #     if noise1.dim() == 2:  
    #         print('📌 Reshaping noise1 from 2D to 4D')
    #         noise1 = noise1.view(noise1.shape[0], noise1.shape[1], 1, 1)
    #         noise1 = noise1.expand(-1, -1, x.shape[2], x.shape[3])

    #     if noise2.dim() == 2:  
    #         print('📌 Reshaping noise2 from 2D to 4D')
    #         noise2 = noise2.view(noise2.shape[0], noise2.shape[1], 1, 1)
    #         noise2 = noise2.expand(-1, -1, x.shape[2], x.shape[3])

    #     # Ensure noise matches x's channel size
    #     if noise1.shape[1] != x.shape[1]:  
    #         noise1 = noise1[:, :x.shape[1], :, :]
    #         noise2 = noise2[:, :x.shape[1], :, :]
    ...
    #     # Ensure istyle is reshaped to [batch_size, 512]
    #     print("istyle shape before Linear layer:", istyle.shape)
    #     istyle = istyle.view(istyle.shape[0], -1).to(device)
    ...
    #     # Handle 128x128 resolution
    #     if x.shape[2] == 128 and x.shape[1] == 256:  
    #         print("⚠️ Adjusting noise for 128x128 resolution after conv1")
    #         noise1 = noise1[:, :256, :, :]  

    #     # Handle 256x256 resolution
    #     if x.shape[2] == 256 and x.shape[1] == 128:  
    #         print("⚠️ Adjusting noise for 256x256 resolution after conv1")
    #         noise1 = noise1[:, :128, :, :]  
    ...
    #     # Handle 128x128 resolution after conv2
    #     if x.shape[2] == 128 and x.shape[1] == 256:
    #         print("⚠️ Adjusting noise for 128x128 resolution after conv2")
    #         noise2 = noise2[:, :256, :, :]  

    #     # Handle 256x256 resolution after conv2
    #     if x.shape[2] == 256 and x.shape[1] == 128:
    #         print("⚠️ Adjusting noise for 256x256 resolution after conv2")
    #         noise2 = noise2[:, :128, :, :]  
    ...
    #     # **Fix: Adjust noise1 and noise2 to match x's channels**
    #     if noise1.shape[1] != x.shape[1]:  # Ensure channel dimension matches
    #         noise1 = noise1[:, :x.shape[1], None, None]  # Resize noise1 to match x's channel size
    #         noise2 = noise2[:, :x.shape[1], None, None]  # Resize noise2 to match x's channel size
    ...
    #     # Ensure style layers match x's channels
    #     style1 = self.to_style1(istyle.to(device))  # Move to device
    #     x = self.conv1(x, style1)
    #     print("x shape:", x.shape)
    #     x = self.activation(x + noise1)  # Ensure noise1 is on the correct device

    #     style2 = self.to_style2(istyle.to(device))  # Move to device
    #     x = self.conv2(x, style2)
    #     x = self.activation(x + noise2)  # Ensure noise2 is on the correct device
    ...
    #     # Fetch the correct noise tensor for the current resolution
    #     current_resolution_index = int(np.log2(x.shape[2])) - 2  # Adjust index based on resolution
    #     inoise = inoise[current_resolution_index]  # Get corresponding noise tensor

    #     # Ensure inoise has the correct shape before passing to Linear layer
    #     inoise = inoise.view(inoise.shape[0], -1)  # Flatten inoise before Linear
    #     expected_noise_features = self.to_noise1.in_features  # Should be 16

    #     if inoise.shape[1] != expected_noise_features:
    #         inoise = F.adaptive_avg_pool1d(inoise.unsqueeze(1), expected_noise_features).squeeze(1)

    #     print(f"inoise shape before Linear layer: {inoise.shape}")
    #     print(f"Expected input size for Linear layer: {expected_noise_features}")

    #     # **Fix: Ensure noise layers output the correct number of channels**
    #     self.to_noise1 = nn.Linear(expected_noise_features, x.shape[1])  # Adapt to `x.shape[1]`
    #     self.to_noise2 = nn.Linear(expected_noise_features, x.shape[1])  # Adapt to `x.shape[1]`

    #     noise1 = self.to_noise1(inoise)
    #     noise2 = self.to_noise2(inoise)

    #     # Reshape to match generator's expected input
    #     noise1 = noise1.view(noise1.shape[0], x.shape[1], 1, 1)  # Match x's channel size
    #     noise2 = noise2.view(noise2.shape[0], x.shape[1], 1, 1)  # Match x's channel size
    ...
    #     # Ensure istyle is reshaped to [batch_size, 512] before passing through Linear layers
    #     print("istyle shape before Linear layer:", istyle.shape)
    #     istyle = istyle.view(istyle.shape[0], -1)

    #     # If `istyle` does not have 512 features, expand or project it to match
    #     if istyle.shape[1] != 512:
    #         istyle = F.interpolate(istyle.unsqueeze(1), size=(512,), mode='linear', align_corners=False).squeeze(1)

    #     print("istyle shape after reshaping:", istyle.shape)

    #     # **Fix: Ensure style layers match x's channels**
    #     self.to_style1 = nn.Linear(512, x.shape[1])  # Adapt to `x.shape[1]`
    #     self.to_style2 = nn.Linear(512, x.shape[1])  # Adapt to `x.shape[1]`

    #     style1 = self.to_style1(istyle)
    #     x = self.conv1(x, style1)
    #     print("x shape downsampled:", x.shape)
    #     x = self.activation(x + noise1)  # **Fix: noise1 shape now matches x**

    #     style2 = self.to_style2(istyle)
    #     x = self.conv2(x, style2)
    #     x = self.activation(x + noise2)  # **Fix: noise2 shape now matches x**
    ...
    #     # Pass through style transformation layers
    #     style1 = self.to_style1(istyle)
    #     x = self.conv1(x, style1)
    #     print("x shape:", x.shape)
    #     x = self.activation(x + noise1)  # **Fix: noise1 shape now matches x**
    ...
    #     # Reshape to match generator's expected input
    #     noise1 = noise1.view(noise1.shape[0], 1, 1, 512).permute((0, 3, 2, 1))
    #     noise2 = noise2.view(noise2.shape[0], 1, 1, 512).permute((0, 3, 2, 1))
    ...
    #     current_resolution_index = int(np.log2(x.shape[2])) - 2  # Adjust index based on resolution
    #     inoise = inoise[current_resolution_index]  # Get corresponding noise tensor
    #     inoise = inoise.view(inoise.shape[0], -1)  # Flatten inoise before passing to Linear
    #     print(f"inoise shape before Linear layer: {inoise.shape}")
    #     print(f"Expected input size for Linear layer: {self.to_noise1.in_features}")

    #     noise1 = self.to_noise1(inoise)  # shape (8, 512)
    #     noise1 = noise1.view(noise1.shape[0], 1, 1, 512)  # Reshape to (8, 1, 1, 512)
    #     noise1 = noise1.permute((0, 3, 2, 1))  # Now it has 4 dimensions

    #     # noise1 = self.to_noise1(inoise).permute((0, 3, 2, 1))
    #     noise2 = self.to_noise2(inoise)  # shape (8, 512)
    #     noise2 = noise2.view(noise2.shape[0], 1, 1, 512)  # Reshape to (8, 1, 1, 512)
    #     noise2 = noise2.permute((0, 3, 2, 1))  # Now it has 4 dimensions
    ...
    #     style2 = self.to_style2(istyle)
    #     # style2 = self.to_style2(istyle.view(8, 512))  # Reshape to (8, 512)
